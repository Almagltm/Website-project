...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($judul) || $id_kategori === 0 || empty($lokasi)) {
        ...
        $stmt = $conn->prepare("INSERT INTO laporan (id_user, id_kategori, judul, isi_laporan, lokasi, instansi_tujuan, tanggal_kejadian, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')");
        $stmt->bind_param('iisssss', $id_user, $id_kategori, $judul, $isi_laporan, $lokasi, $instansi_tujuan, $tanggal_kejadian);
        if ($stmt->execute()) {
            $id_laporan = $conn->insert_id;
            $stmt->close();

            // Update foto_awal jika kolom tersedia
            $conn->query("UPDATE laporan SET foto_awal = '" . $conn->real_escape_string($foto_awal_db) . "' WHERE id_laporan = $id_laporan");
  ...
  <div class="form-card">
    ...
    <select id="id_kategori"
        ...
        <?php foreach ($kategori_list as $k): ?>
            <option
                value="<?= $k['id_kategori'] ?>"
                <?= (($_POST['id_kategori'] ?? '') == $k['id_kategori']) ? 'selected' : '' ?>>
    ...
    <div id="lainnya-wrap" style="display:none; margin-top:10px;">
        <input
            type="text"
            id="kategori_lainnya"
            name="kategori_lainnya"
            placeholder="Tuliskan kategori Anda..."
            value="<?= htmlspecialchars($_POST['kategori_lainnya'] ?? '') ?>">
    </div>
</div>
        <div class="fg">
